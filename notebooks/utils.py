...
from risf.distance import SelectiveDistance, TrainDistanceMixin, split_distance_mixin
...
def get_dataset(type_, data_folder, name, numerical_features=False) -> dict:
    if type_ == "numerical":
...
def experiment_risf_complex(
    data, distances, selected_obj_ratio, selection_func=selection_choice, clf_kwargs={}
):
    distances_path = [
...
def perform_experiment_simple(
    clf_name, data, distances, clf_kwargs, dtype_="numerical"
):
    X, y = data["X"], data["y"]
    all_indices = np.arange(X.shape[0])
    auc = []
