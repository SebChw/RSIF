...
class DistanceMixin():
    ...
    def precompute_distances(self, X, selected_objects=None):
        ...
        n = len(selected_objects) # number of objects that can constitute a pair
        ...
        all_objects = self.get_all_objects_ids_with_selected_at_front(m, selected_objects)
...
class TestDistanceMixin:
    ...
    def precompute_distances(self, X_train, X_test):
        self.distance_matrix = np.zeros((X_train.shape[0], X_test.shape[0])) #! We create bigger array than needed but this is still fine.
